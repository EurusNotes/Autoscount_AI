...
| **Otta** | `/jobs/` 路径 |
| **Hatch** | `/jobs/` 或 `/role/` 路径 |
| **Prosple** | `/graduate-jobs/`、`/internships/`、`/jobs/` 路径 |

### 其他任意网站（自动检测）

对于不在上述列表中的网站，插件通过两种方式自动识别职位页，无需任何配置：

1. **Schema.org 结构化数据** — 许多招聘网站和企业官网招聘页会在页面源码中嵌入 `JobPosting` 数据块，插件可直接读取，提取结果干净可靠。
2. **关键词启发式检测** — 若页面文本包含足够多的招聘相关语言（如「responsibilities」、「requirements」、「apply now」等），则将该页面识别为职位页。

检测到职位页后，绿色「**分析此职位**」按钮会自动出现。非职位页面不会注入任何界面元素。
